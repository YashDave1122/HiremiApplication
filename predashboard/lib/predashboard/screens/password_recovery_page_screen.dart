...
class _PasswordRecoveryPageScreenState
  ...
  void handleErrorResponse(BuildContext context, String responseBody) {
    final Map<String, dynamic> responseJson = jsonDecode(responseBody);

    String errorMessage = 'Unknown error';
    if (responseJson.containsKey('non_field_errors')) {
      errorMessage = responseJson['non_field_errors'][0];
    }
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () async {
                    if (_formKey.currentState?.validate() ?? false) {
                      ...
                      // final response = await _userService.createPostApi(body, ApiUrls.SendOtpForEmailValidation);
