...
from .models import Account, City, CorporateDiscount, CorporateTraining, Discount, EmailOTP, LiveProjectHub, LiveProjectHubApplication, Mentorship, MentorshipDiscount, PasswordResetOTP, State, TrainingProgram, TrainingProgramApplication
from .permissions import IsOwner, IsOwnerOrReadOnly, IsSelf, IsSelfOrReadOnly
from .serializers import (AccountLoginSerializer, AccountLogoutSerializer,
                          AccountRegisterSerializer, AccountSerializer,
                          CitySerializer, CorporateDiscountSerializer, CorporateSerializer, DiscountSerializer, GenerateOTPSerializer,
                          GeneratePasswordResetOTPSerializer, LiveProjectHubApplicationSerializer, LiveProjectHubSerializer, MentorshipDiscountSerializer, MentorshipSerializer,
                          ResetPasswordSerializer, StateSerializer, TrainingProgramApplicationSerializer, TrainingProgramSerializer,
                          VerifyOTPSerializer,
                          VerifyPasswordResetOTPSerializer)
from .utils import (generate_refresh_response, generate_token_response,
                    send_login_otp_to_email, send_password_reset_otp_to_email,
...
@api_view(['GET'])
def get_city(request, state_name, city_id):
    try:
        state = State.objects.get(name=state_name)
        ...
        return Response({"error": "City not found in the given state"}, status=404)
...
class MentorshipViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            register = Account.objects.get(id=register_id)
...
class LiveProjectViewSet(viewsets.ModelViewSet):
    queryset = LiveProjectHub.objects.all()
    serializer_class = LiveProjectHubSerializer


class LiveProjectApplicationViewSet(viewsets.ModelViewSet):
    queryset = LiveProjectHubApplication.objects.all()
    serializer_class = LiveProjectHubApplicationSerializer

    # Override the create method to check if the user already applied
    def create(self, request, *args, **kwargs):
        register = request.data.get('register')
        LiveProjectProgram = request.data.get('LiveProjectProgram')
        
        # Check if the application already exists
        if LiveProjectHub.objects.filter(register=register, LiveProjectProgram=LiveProjectProgram).exists():
            return Response({"error": "You have already applied for this LiveProjectProgram program."}, status=status.HTTP_400_BAD_REQUEST)
